...
$DOCKER_CMD run -d \
  ...
  PHPMYADMIN_CONTAINER_NAME="$DB_CONTAINER_NAME-phpmyadmin"
  PHPMYADMIN_PORT="8080"
  
  if [ "$($DOCKER_CMD ps -q -f name=$PHPMYADMIN_CONTAINER_NAME)" ]; then
    echo "phpMyAdmin container '$PHPMYADMIN_CONTAINER_NAME' already running"
  elif [ "$($DOCKER_CMD ps -q -a -f name=$PHPMYADMIN_CONTAINER_NAME)" ]; then
    $DOCKER_CMD start "$PHPMYADMIN_CONTAINER_NAME"
    echo "Existing phpMyAdmin container '$PHPMYADMIN_CONTAINER_NAME' started"
  else
    $DOCKER_CMD run -d \
      ...
      -e PMA_HOST=db \
      docker.io/phpmyadmin/phpmyadmin && echo "phpMyAdmin container '$PHPMYADMIN_CONTAINER_NAME' was successfully created"
  fi
